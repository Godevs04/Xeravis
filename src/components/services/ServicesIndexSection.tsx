...
const ICONS_BY_SLUG: Record<string, LucideIcon> = {
  'artificial-intelligence-ai-research': Sparkles,
  'data-science-advanced-analytics': BarChart3,
  'it-consulting-digital-transformation': Briefcase,
  'clinical-data-science-healthcare-ai': HeartPulse,
  'data-engineering-cloud-solutions': Cloud,
}
...
function iconFor(service: ServiceItem): LucideIcon {
  if (service.icon && ICONS_BY_NAME[service.icon]) return ICONS_BY_NAME[service.icon]
  return ICONS_BY_SLUG[service.slug] ?? Layers
}

export function ServicesIndexSection({ services }: ServicesIndexSectionProps) {
              ...
              >
                <Link
                  ...
                  className={cn(
                    ...
                    'sm:flex-row sm:items-center sm:justify-between sm:gap-8 sm:p-6 lg:p-7',
                ...
                >
                  ...
                  <div className="relative flex min-w-0 items-start gap-4 sm:items-center">
                    <span className="font-display hidden w-10 shrink-0 text-sm font-bold tracking-[0.14em] text-[color:var(--color-accent)] sm:block">
                    ...
                    <div className="min-w-0">
                      <span className="font-display text-[10px] font-bold tracking-[0.16em] text-[color:var(--color-muted)] uppercase sm:hidden">
                        {String(index + 1).padStart(2, '0')}
                      </span>
                      <h3 className="font-display mt-1 text-xl font-semibold tracking-tight text-[color:var(--color-primary)] transition-colors group-hover:text-[color:var(--color-accent)] sm:mt-0 sm:text-2xl">
                        {service.title}
                      </h3>
                    </div>
                  </div>

                  <div className="relative flex items-end justify-between gap-4 sm:max-w-md sm:items-center sm:justify-end lg:max-w-lg">
